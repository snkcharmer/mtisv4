<script type="text/javascript">
    ...
    $('#employid').select2({
        placeholder: "Select a Employeer",
        allowClear: true
    });
   ...
   /* $('.provid').select2({
	...
	/*$(document).on("change", ".regid", function(event){

    	document.getElementById('provid').options.length = 0; //erase dropdown
    	document.getElementById('mcid').options.length = 0; //erase dropdown
    	$("#pcode").val("");
		$.ajax({
		   url:"<?php// echo base_url(); ?>nea/get_province_/",
		   method:"post",
		   dataType:"json",
		   data:{
			   'regid': $('.regid').val()
		   ...
		   success:function(data)
		   {
			   ...
			   $("#provid").append("<option value=''>Select a Province</option>");
			   for(var i = 0; i < data.prov.length; i++){
			   		$("#provid").append("<option value='"+data.prov[i]['province']+"'>"+data.prov[i]['province']+"</option>");
			   } 
		   }
		});
	...
	$(document).on("change", ".provid", function(event){

    	document.getElementById('mcid').options.length = 0; //erase dropdown

		$.ajax({
		   url:"<?php //echo base_url(); ?>nea/get_municipality_/",
		   method:"post",
		   dataType:"json",
		   data:{
			   'provid': $('.provid').val()
		   ...
		   success:function(data)
		   {
			   ...
			   $("#mcid").append("<option value=''>Select a Municipality</option>");
			   for(var i = 0; i < data.muni.length; i++){
			   		$("#mcid").append("<option value="+data.muni[i]['idnum']+">"+data.muni[i]['municipal']+"</option>");
			   } 
		   }
		});
	...
	/*$(document).ready(function () {
	    ...
	    allNextBtn.click(function () {
	        var curStep = $(this).closest(".setup-content"),
	            ...
	            curInputs = curStep.find("input[type='text'],input[type='radio'],input[type='date'],select,input[type='number'],input[type='email']"),
	            isValid = true;
	...
	function lsesave(){
			...
			$.ajax({
			   ...
			   success:function(data)
			   {
				   if(data.rec['seid'] == 'error'){
				   		if($('#licid').val() == "" && $('#rankid').val() == "" && $('#datedis').val() == "" && $('#employid').val() == "" && $('#sprin').val() == "" && $('#lnum').val() == ""){
				   		...
				   		swal("Alert!", "Data Successfully added", "success");
					   	$('#seid').val(data.rec['seid']);
				   }
			   }
			});
		...
		$.ajax({
		   ...
		   success:function(data)
		   {
			   if(data.rec['courseid'] == 'error'){
			   		...
			   		swal("Alert!", "Data Successfully added", "success");
				   	$('#csid').val($('.courseid').val());
				   	$('#schoolid').val($('.schlid').val());
			   }
		   }
		});
