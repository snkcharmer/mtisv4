...
class nea_model extends CI_Model
{
		...
		function add_gin(){
			...
			$data = [
				...
				'citizenship' => $this->input->post('citi'),
				'locid' => $locationid,
				'landline' => $this->input->post('lnumber'),
				'mobilenum' => $mobile,
				'emailadd' => $this->input->post('emailacc'),
				'fbaccount' => $this->input->post('fbacc'),
				'entrydate' => date('Y-m-d')
			];

			if($this->input->post('trid') == ""){
				$this->db->insert('trainee_',$data);
		...
		function add_location($locid = null){

			if($locid == "" || $locid == null){
				$data = [
					//'housenum' => $this->input->post('street'),
					'brgy' => $this->input->post('address'),
					'mcid' => $this->input->post('mcid'),
					'province' => $this->input->post('provid'),
					'region' => $this->input->post('regid'),
					'code' => $this->input->post('pcode')
				];
				$this->db->insert('location_',$data);
		...
		function add_cpcesave(){
			...
			$cpid = $this->add_contact_person($this->input->post('cpid'));
			$data = [
				'cpid' => $cpid
			];
		...
		function add_contact_person($cpid = null){

			if($this->input->post('mob2') != ""){
				$mobile = $this->input->post('mob1').' / '.$this->input->post('mob2');
			}else{
			...
			if($cpid == "" || $cpid == null){
				$data = [
					'efullname' => $this->input->post('fullname'),
					'relationship' => $this->input->post('rel'),
					'ecomaddress' => $this->input->post('caddress'),
					'elandline' => $this->input->post('telnum'),
					'emobilenum' => $mobile,
					'eemailadd' => $this->input->post('emailadd')
				];
				$this->db->insert('emergency_con_person_',$data);
				...
				$this->db->update('emergency_con_person_',$data);
		...
		function add_lsesave(){
			...
			$seid = $this->add_license_name($this->input->post('seid'));
			$data = [
				'seid' => $seid
			];
		...
		function add_license_name($seid = null){
			...
			if($seid == "" || $seid == null){
				$data = [
					...
					'shipprinid' => $this->input->post('employid'),
					'companyname' => $this->input->post('sprin'),
					'selandline' => $this->input->post('lnum'),
					'semobilenum' => $this->input->post('mnum')
				];
				$this->db->insert('shipboard_experience',$data);
				$id = $this->db->insert_id();

				$datae = [
					'shipowner' => $this->input->post('sprin'),
					'telnum' => $this->input->post('lnum'),
					'mnumber' => $this->input->post('lnum')
					];
				$this->db->where('employid',$this->input->post('employid'));
				$this->db->update('employer',$datae);
				...
				$this->db->update('shipboard_experience',$data);
